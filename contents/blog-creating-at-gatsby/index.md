...
本題とは少し話がそれますが、Gatsby.js を少しずつ触るようになって「Gatsby.js でブログを作ってみた」系の技術ブログをよく拝見するようになったのですが、ほとんどが口を揃えて **Gatsby.js は SEO につよい** と書いています。

**SEO に強い = 強豪キーワードで上位表示できるのか？**

もちろんほとんどの方は、Gatsby.js で構築した技術ブログの内容は Gatsby.js を含め、技術に関する事が書かれている事が多い。Gatsby 〇〇といったキーワードなら上位表示されているがそれ以外のキーワードで上位表示されているサイトや記事を（私の場合は）あまり見かけない。<p class="danger">言わずもがなですが、人によって検索キーワードや閲覧するサイトは同じではないです</p>

私が閲覧するほとんどのサイトの場合、検索結果の上位表示を占めているのは WordPress で構築されたサイトであり、いわゆる強豪キーワードで Gatsby.js で構築されたサイトを見かけた事がない。それも踏まえ、検索ボリュームがそこそこあるかつ、Web に関する良い商品やサービスがあれば狙って上位表示できるのかを検証して色々試してみたい。

ページの表示速度が SEO に影響すると言われているのもわかりますし、実際遅いと離脱率が上がるのもわかります。しかし上位表示されている WordPress で構築されたサイトの Lighthouse のスコアは平均して低いサイトが多い印象を受けます。（勿論全てがそうではありません）

上位表示される要素は他にもドメインパワーやライティング、内部対策、被リンク数などあらゆる要素の合計値だとわかるが、それらも踏まえて色々検証していきたい。

長々とタイトルとは話がそれましたが、上記を踏まえてこの技術選定に至りました。ちなみに Tailwind CSS とかは使いやすいと思っているのでそこは関係ありません。
