...
<!-- HTML や CSS を使い、要素を横並びにしたい時、いくつかの選択肢があります。

1. HTML の `<table>` タグを使い、要素を横並びにする
1. CSS の `float` で要素を横並びにする
1. CSS の `display: inline-block;` で要素を横並びにする
1. CSS の `display: flex;` で要素を横並びにする
1. CSS の `display: grid;` で要素を横並びにする

今回は、③ の `display: flex;` について解説していきます。

---

（① に関してですが、HTML の table タグでレイアウトを組むというのは最近学習し始めた方だと想像がつかないかと思いますが、昔の Web の制作現場ではサイトのレイアウトを構築していく際、その table タグを使ってレイアウトを組んでいました。現在、CSS で表現できる幅も広がった為、そのような現場は少ないと思いますが、制作現場に入って初めてみた方が戸惑わないよう、このような形で記載しています。これから HTML や CSS を学ぶ方は、こういう方法でレイアウトを組んでいた時代もあったんだなと頭に入れとくのもいいかもしれません） -->
